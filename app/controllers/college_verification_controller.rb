  ...
  def completed
    if params.has_key?(:search_tag)
      @college_verifications_completed =  VerificationRequest.all.where(
        " payment_id NOT null AND college_id = ? AND verification_status_id > ? AND (hallticket_no ILIKE ? OR name ILIKE ? OR verification_token ILIKE ?)", 
          @college_id, 
          2, 
          "%#{params[:search_tag]}%",
          "%#{params[:search_tag]}%",
      ...
      @college_verifications_completed =  VerificationRequest.all.where(
        " payment_id NOT null AND college_id = ? AND verification_status_id > ?", 
          @college_id, 
          2
        ).order('created_at DESC')
      .paginate(page: params[:page],:per_page => 10)
